...
import { Account, Address, Asset, Contract, Keypair, Memo, Networks, Operation, TransactionBuilder, nativeToScVal } from '@stellar/stellar-sdk'
...
function xdr(operations: TestOperation[], memo?: Memo) {
  const builder = new TransactionBuilder(new Account(SOURCE, '0'), { fee: '100', networkPassphrase: Networks.TESTNET })
...
describe('transaction review schema', () => {
  it('has an explicit SDK compatibility gate for every pinned operation discriminant', () => {
    expect(PINNED_SDK_OPERATION_TYPES).toEqual([
      ...
      'claimClaimableBalance', 'clawback', 'clawbackClaimableBalance', 'createAccount', 'createClaimableBalance',
      'createPassiveSellOffer', 'endSponsoringFutureReserves', 'extendFootprintTtl', 'inflation', 'invokeHostFunction',
      'liquidityPoolDeposit', 'liquidityPoolWithdraw', 'manageBuyOffer', 'manageData', 'manageSellOffer',
      'pathPaymentStrictReceive', 'pathPaymentStrictSend', 'payment', 'restoreFootprint', 'revokeAccountSponsorship',
      'revokeClaimableBalanceSponsorship', 'revokeDataSponsorship', 'revokeLiquidityPoolSponsorship',
      'revokeOfferSponsorship', 'revokeSignerSponsorship', 'revokeTrustlineSponsorship', 'setOptions', 'setTrustLineFlags',
    ])
  })
  ...
  it('preserves memo, amount, issuer, path, source, exact network and digest', () => {
    const review = extractTransactionReview(xdr([
      Operation.pathPaymentStrictSend({ sendAsset: Asset.native(), sendAmount: '10', destination: DESTINATION, destAsset: new Asset('USD', ISSUER), destMin: '9', path: [Asset.native()] }),
    ], Memo.text('invoice-42')), Networks.TESTNET)
    expect(review).not.toBeNull()
    expect(review).toMatchObject({ networkPassphrase: Networks.TESTNET, memo: { value: 'invoice-42' }, envelope: { source: SOURCE } })
    expect(review?.xdrDigest).toMatch(/^[a-f0-9]{64}$/)
    expect(review?.operations[0].facts).toEqual(expect.arrayContaining([
  ...
  it('does not let a low score suppress an account-authority finding', () => {
    ...
    const aggregate = aggregateReview(review, [{ target: { type: 'account', value: DESTINATION }, score: 1, status: 'available' }])
  ...
  it('retains claimable balance IDs as typed review targets and never scores them as accounts', () => {
    const review = extractTransactionReview(xdr([Operation.claimClaimableBalance({ balanceId: BALANCE_ID })]), Networks.TESTNET)!
    expect(review.operations[0].targets).toEqual([{ type: 'claimable-balance', value: BALANCE_ID }])
    expect(scoreableTargets(review)).toEqual([])
  })

  it('preserves fee payer separately from the inner transaction source', () => {
    const inner = new TransactionBuilder(new Account(SOURCE, '0'), { fee: '100', networkPassphrase: Networks.TESTNET })
    ...
    const feeBump = TransactionBuilder.buildFeeBumpTransaction(feePayer, '1000', inner, Networks.TESTNET)
  ...
  it('preserves all 100 operations in exact order', () => {
    const operations = Array.from({ length: 100 }, (_, index) => Operation.manageData({ name: `key-${index}`, value: String(index) }))
    const review = extractTransactionReview(xdr(operations), Networks.TESTNET)!
    expect(review.operations).toHaveLength(100)
    expect(review.operations.map((operation) => operation.index)).toEqual(Array.from({ length: 100 }, (_, index) => index))
  })
  ...
  it('integrates Soroban transfer meaning and provenance into the review', () => {
    ...
    const review = extractTransactionReview(xdr([
      new Contract(contract).call('transfer', new Address(SOURCE).toScVal(), new Address(DESTINATION).toScVal(), nativeToScVal(12_000_000n, { type: 'i128' })),
    ]), Networks.TESTNET)!
    expect(review.operations[0]).toMatchObject({ type: 'invokeHostFunction', coverage: 'understood' })
    expect(review.operations[0].facts).toEqual(expect.arrayContaining([
    ...
    expect(review.operations[0].targets).toEqual(expect.arrayContaining([{ type: 'contract', value: contract }, { type: 'account', value: DESTINATION }]))
  })

  it('returns a finite non-green outcome for arbitrary malformed XDR strings', () => {
    fc.assert(fc.property(fc.string({ maxLength: 2048 }), (value) => {
